...
struct CreatePlayerButton: View {
    @Environment(\.grdbManager) private var grdbManager
...
struct DeletePlayersButton: View {
    ...
    var body: some View {
        Button {
            switch mode {
            case .delete:
                _ = try! GRDBManager.shared.playerRepository.deleteAllPlayer()
//                _ = try! grdbManager.playerRepository.deleteAllPlayer()
                
            case let .deleteAfter(action):
                action()
                Task {
                    try await Task.sleep(nanoseconds: 100_000_000)
                    try grdbManager.playerRepository.deleteAllPlayer()
                }
